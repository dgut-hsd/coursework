...
fn main() -> Result<()> {
    ...
    // 不同 batch size 用不同的测试轮数，控制总时间
    fn get_rounds(bs: usize) -> usize {
        match bs {
            1  => 100,
            4  => 50,
            16 => 30,
    ...
    println!("{:-^80}", " Benchmark 报告 ");
    println!(
        "{:>10}  {:>15}  {:>12}  {:>12}  {:>12}",
        "Batch", "Sentences/s", "Avg Lat(ms)", "P50(ms)", "P99(ms)"
    );
    println!("{:=<10}  {:=<15}  {:=<12}  {:=<12}  {:=<12}", "", "", "", "", "");

    for &bs in &batch_sizes {
        // 构造 bs 条测试文本
        let texts: Vec<String> = (0..bs)
            .map(|i| format!("测试文本 {}：用于 Benchmark 的示例条文第 {} 条", i + 1, i + 1))
            .collect();
        ...
        // 统计
        let total_sec = latencies.iter().map(|d| d.as_secs_f64()).sum::<f64>();
        let sentences_per_sec = (bs * n_rounds) as f64 / total_sec;
        let avg_lat = total_sec / n_rounds as f64 * 1000.0; // ms

        latencies.sort();
        let p50 = latencies[(n_rounds as f64 * 0.50) as usize];
        let p99_idx = (n_rounds as f64 * 0.99).min((n_rounds - 1) as f64) as usize;
        let p99 = latencies[p99_idx];

        println!(
            "{:>10}  {:>15.0}  {:>12.2}  {:>12.2}  {:>12.2}",
            bs,
            sentences_per_sec,
            avg_lat,
    ...
    println!("\n✅ 任务 4 完成：Benchmark 数据已输出。");
    println!("  - 当前为 debug 模式，结果仅供参考。");
    println!("  - 运行 `cargo run --release -- <模型路径>` 可获得更准确的性能数据。");
    println!("  - 内存占用可用 Windows 任务管理器观察。");
    Ok(())
}
