...
impl EmbeddingEngine {
    ...
    pub fn load(model_dir: &str) -> Result<Self> {
        //1. 文件路径
        let model_path = format!("{}/model_fp16.onnx", model_dir);
        ...
        for j in 0..hidden_dim {
            ...
            for i in 0..seq_len {
                if attention_mask[[b, i]] != 0 {
                    let val = hidden_states[[b, i, j]];
                    if val > max_val {
                        max_val = val;
                    }
                }
            }
            result[[b, j]] = if max_val > f32::MIN { max_val } else { 0.0 };
        }

        // L2 normalize
        let norm_sq: f32 = (0..hidden_dim).map(|j| result[[b, j]].powi(2)).sum();
        let norm = norm_sq.sqrt();
        if norm > 0.0 {
            for j in 0..hidden_dim {
                result[[b, j]] /= norm;
...
/// ONNX 推理输出的两种形态
enum OnnxOutput {
    /// 3D: [batch, seq_len, 1024] — token 级输出，需要 pooling
    TokenLevel(Array3<f32>),
    /// 2D: [batch, 1024] — 模型已内置池化，只需 L2 归一化
    SentenceLevel(Array2<f32>),
}

/// 对 2D 数组逐行做 L2 归一化
fn l2_normalize_2d(input: &Array2<f32>) -> Array2<f32> {
    let (batch, hidden_dim) = input.dim();
    let mut result = Array2::<f32>::zeros((batch, hidden_dim));
    for b in 0..batch {
        let row = input.slice(s![b, ..]);
        let norm_sq: f32 = row.iter().map(|x| x * x).sum();
        let norm = norm_sq.sqrt();
        if norm > 0.0 {
            for (j, &v) in row.iter().enumerate() {
...
impl EmbeddingEngineInner {
    /// 共享推理逻辑：ndarray → ONNX → 原始输出
    /// 自动适配 3D（token 级）和 2D（已池化）输出
    fn onnx_infer(
        &self,
        input_ids: &Array2<i64>,
        attention_mask: &Array2<i64>,
    ...
    /// Mean Pooling 推理（保留原逻辑，兼容 2D 输出）
    fn run_onnx_batch(
        &self,
        input_ids: &Array2<i64>,
        attention_mask: &Array2<i64>,
        ...
        match self.onnx_infer(input_ids, attention_mask, token_type_ids)? {
            OnnxOutput::TokenLevel(hidden) => Ok(mean_pooling(&hidden, attention_mask)),
            OnnxOutput::SentenceLevel(emb) => Ok(l2_normalize_2d(&emb)),
        }
    }
    ...
    fn run_onnx_cls(
        ...
        match self.onnx_infer(input_ids, attention_mask, token_type_ids)? {
            OnnxOutput::TokenLevel(hidden) => Ok(cls_pooling(&hidden)),
            // 模型已池化，退化为 L2 归一化
            OnnxOutput::SentenceLevel(emb) => Ok(l2_normalize_2d(&emb)),
        }
    }
    ...
    fn run_onnx_max(
        ...
        match self.onnx_infer(input_ids, attention_mask, token_type_ids)? {
            OnnxOutput::TokenLevel(hidden) => Ok(max_pooling(&hidden, attention_mask)),
            // 模型已池化，退化为 L2 归一化
            OnnxOutput::SentenceLevel(emb) => Ok(l2_normalize_2d(&emb)),
        }
    }
...
impl EmbeddingEngine {
    ...
    pub fn embed_batch_cls(&self, texts: &[&str]) -> Result<Vec<[f32; HIDDEN_DIM]>> {
        if texts.is_empty() {
            return Ok(vec![]);
        }

        let inner = &self.inner;
        let mut all_embeddings: Vec<[f32; HIDDEN_DIM]> = Vec::with_capacity(texts.len());

        for chunk in texts.chunks(inner.batch_size) {
            let tokenized = inner.tokenize_batch(chunk)?;
            let embeddings = inner.run_onnx_cls(
                &tokenized.input_ids,
                &tokenized.attention_mask,
                &tokenized.token_type_ids,
            )?;
    ...
    pub fn embed_batch_max(&self, texts: &[&str]) -> Result<Vec<[f32; HIDDEN_DIM]>> {
        if texts.is_empty() {
            return Ok(vec![]);
        }
        ...
        for chunk in texts.chunks(inner.batch_size) {
            let tokenized = inner.tokenize_batch(chunk)?;
            let embeddings = inner.run_onnx_max(
                &tokenized.input_ids,
                &tokenized.attention_mask,
                &tokenized.token_type_ids,
            )?;
...
fn main() -> Result<()> {
    ...
    // Mean Pooling
    {
        ...
        for pair in &pairs {
            ...
            cosines.push(cosine(&emb_a[0], &emb_b[0]));
        }
        results.push(PoolResult { name: "Mean", cosines });
    }
    ...
    // CLS Pooling
    {
        let mut cosines = Vec::with_capacity(pairs.len());
        for pair in &pairs {
            let emb_a = engine.embed_batch_cls(&[&pair.a])?;
            let emb_b = engine.embed_batch_cls(&[&pair.b])?;
            cosines.push(cosine(&emb_a[0], &emb_b[0]));
        }
        results.push(PoolResult { name: "CLS", cosines });
    }
    ...
    // Max Pooling
    {
        let mut cosines = Vec::with_capacity(pairs.len());
        for pair in &pairs {
            let emb_a = engine.embed_batch_max(&[&pair.a])?;
            let emb_b = engine.embed_batch_max(&[&pair.b])?;
            cosines.push(cosine(&emb_a[0], &emb_b[0]));
        }
        results.push(PoolResult { name: "Max", cosines });
    }

    // ── 打印对比表（每 10 对输出一次） ──
    println!("{:-^80}", " 三种 Pooling 策略相似度对比 ");
    println!(
        "{:>5}  {:>10}  {:>10}  {:>10}  {:>10}",
        "#", "Human", "Mean", "CLS", "Max"
    );
    ...
    for i in 0..pairs.len() {
        if i % 10 == 0 || i == pairs.len() - 1 {
            println!(
                "{:>5}  {:>10.2}  {:>10.4}  {:>10.4}  {:>10.4}",
                i + 1,
                pairs[i].human_score,
                results[0].cosines[i],
                results[1].cosines[i],
    ...
    // ── 统计摘要 ──
    let mean_stats = results[0].cosines.iter().copied().collect::<Vec<_>>();
    let cls_stats  = results[1].cosines.iter().copied().collect::<Vec<_>>();
    let max_stats  = results[2].cosines.iter().copied().collect::<Vec<_>>();

    let avg = |v: &[f32]| v.iter().sum::<f32>() / v.len() as f32;
    let min = |v: &[f32]| v.iter().cloned().fold(f32::MAX, f32::min);
    let max_f = |v: &[f32]| v.iter().cloned().fold(f32::MIN, f32::max);
    ...
    let human_vals = pairs.iter().map(|p| p.human_score).collect::<Vec<_>>();
    println!(
        "{:>12}  {:>10.2}  {:>10.4}  {:>10.4}  {:>10.4}",
        "均值", avg(&human_vals),
        avg(&mean_stats), avg(&cls_stats), avg(&max_stats)
    );
    println!(
        "{:>12}  {:>10.2}  {:>10.4}  {:>10.4}  {:>10.4}",
        "最小值", min(&human_vals),
        min(&mean_stats), min(&cls_stats), min(&max_stats)
    );
    println!(
        "{:>12}  {:>10.2}  {:>10.4}  {:>10.4}  {:>10.4}",
        "最大值", max_f(&human_vals),
        max_f(&mean_stats), max_f(&cls_stats), max_f(&max_stats)
    );

    // ── Mean 与 CLS/Max 的差异分布 ──
    println!("\n{:-^80}", " Mean 与 CLS / Max 的差异 ");
    let diff_cls: Vec<f32> = mean_stats.iter().zip(cls_stats.iter())
        .map(|(m, c)| (m - c).abs()).collect();
    let diff_max: Vec<f32> = mean_stats.iter().zip(max_stats.iter())
        .map(|(m, x)| (m - x).abs()).collect();
    println!(
        "Mean vs CLS:  avg_diff={:.4}, max_diff={:.4}",
        avg(&diff_cls), max_f(&diff_cls)
    );
    println!(
        "Mean vs Max:  avg_diff={:.4}, max_diff={:.4}",
        avg(&diff_max), max_f(&diff_max)
    );

    // ── 与人工标注的 Spearman ──
    println!("\n{:-^80}", " 与 Human Score 的 Spearman 相关系数 ");
    for r in &results {
        let rho = spearman_rho(&r.cosines, &human_vals);
        println!("  {:<5}  ρ = {:.4}", r.name, rho);
    }

    // ── 模型形态说明 ──
    if results[0].cosines == results[1].cosines && results[1].cosines == results[2].cosines {
        println!(
            "\n⚠️  当前模型输出为 2D（已内置池化），三种策略结果完全相同。\n\
              要看到真正的差异，需使用输出 raw 3D hidden states 的 ONNX 模型\n\
              （如原始导出版 BGE-M3，非 sentence-similarity 量化版）。"
        );
    }

    println!("\n✅ 任务 3 完成：三种 Pooling 策略的相似度数据已输出。");
    Ok(())
}
